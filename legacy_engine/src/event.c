...
/* aEventBit is a file-scope global used by the event subsystem.
 *
 * RL-training note: today the engine is a single-instance process
 * (one Junqi per OS process), so a global bitmap is safe - every
 * OpenEngine() memsets it. If we later embed the core as a Python
 * library with multiple in-process Junqi instances (self-play),
 * we must migrate this to Junqi::aEventBit and plumb pJunqi through
 * the 20+ call sites below. The per-instance field already exists. */
u8 aEventBit[100] = {0};
