...
//		}
		if( !(sum>250&&sum<=256) )
		{
			/* Probability sum should be 256 (256/256=100%). A deviation
			 * indicates an accounting bug in dark-chess probabilities.
			 * Previously slept 1s then asserted; in RL self-play this
			 * would crash + block. Now log and clamp. */
			LOG_ERROR(LOG_CAT_PROB,
			          "percent sum out of range: %d (aPercent=%d,%d,%d)",
			          sum, aPercent[0], aPercent[1], aPercent[2]);
