...
void MakeNextMove(Junqi *pJunqi, MoveResultData *pResult)
{
	...
	if( pSrc->type==NONE )
	{
	    /* Previously: log_c("debug"); sleep(1);
	     * These debug leftovers froze the engine for 1 second
	     * whenever a stale move was processed. Removed for RL. */
	    LOG_ERROR(LOG_CAT_RULE, "MakeNextMove: src at (%d,%d) is NONE (stale move?)",
	              p1.x, p1.y);
	}
...
/* Runtime-configurable search timeout in seconds.
 * Default 5s suits interactive play; RL self-play can lower it to
 * sub-second, or raise it for deeper analysis. */
int g_search_timeout_sec = 5;
...
int TimeOut(Junqi *pJunqi)
{
	...
	if( pJunqi->bMove || pJunqi->bGo || search_time > g_search_timeout_sec )
	{
		rc = 1;
	}
...
void PrintBestMove(BestMove *aBestMove, int alpha, int depth)
{
    ...
    for(p=aBestMove[0].pHead; p!=NULL; p=p->pNext)
    {
        ...
        for(int j=0;j<4;j++)
        {
            ...
            LOG_TRACE(LOG_CAT_SEARCH, "depth %d val %d per %d", i, alpha, p->result[j].percent);
...
//                    !memcmp(&aBeasMove[1].pTest->move,test3,4))
            ...
            {
                LOG_TRACE(LOG_CAT_SEARCH, "best cnt %d val %d per %d", cnt, val, aBestMove[cnt-1].pTest->percent);
                SafeMemout((u8*)&aBestMove[cnt-1].pTest->move, sizeof(MoveResultData));
            }
        ...
        if( val>mxVal )
        {
            ...
            {
                LOG_TRACE(LOG_CAT_SEARCH, "SearchBestMove: pNode NULL, allocating tail");
            ...
            if( val>alpha )
            {
                ...
                LOG_TRACE(LOG_CAT_SEARCH, "alpha1: %d depth %d", alpha, cnt);
            }
        }
        ...
        if( 1==cnt )
        {
            ...
            LOG_TRACE(LOG_CAT_SEARCH, "SearchBestMove end");
        }
		...
		int beta)
{
	...
	int val;
	int sum = 0;
	int cnt = ++pJunqi->pEngine->searchCnt;
	int iDir = pJunqi->eTurn;
