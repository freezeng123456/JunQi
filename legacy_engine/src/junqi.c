...
Junqi *JunqiOpen(void)
{
	...
	if (!pJunqi) {
		fprintf(stderr, "[JunqiOpen] malloc failed\n");
		return NULL;
	}
	memset(pJunqi, 0, sizeof(Junqi));
	pJunqi->iEngineDir = ENGINE_DIR_DEFAULT;
	pJunqi->preTurn    = 1000;  /* sentinel: "no previous turn yet" */
	gJunqi = pJunqi;
	pthread_mutex_init(&pJunqi->mutex, NULL);
	LOG_INFO(LOG_CAT_CORE, "Junqi engine opened (seat=%d)", pJunqi->iEngineDir);
	return pJunqi;
}
...
void SetChess(Junqi *pJunqi, enum ChessDir dir)
{
	...
	for(i=0;i<30;i++)
	{
		...
		(void)iType; /* referenced only in debug assertion */
        // 下家最右侧横坐标为0
		// 对家最上面纵坐标为0
		switch(dir)
		{
...
void DestroyAllChess(Junqi *pJunqi, int iDir)
{
	...
	LOG_INFO(LOG_CAT_CORE, "DestroyAllChess: player %d eliminated", iDir);
	...
	if( !pJunqi->bSearch )
	{
		...
		{
			pJunqi->bStart = 0;
			LOG_INFO(LOG_CAT_CORE, "GAME OVER: team [0,2] dead=(%d,%d) team [1,3] dead=(%d,%d)",
			         pJunqi->aInfo[0].bDead, pJunqi->aInfo[2].bDead,
			         pJunqi->aInfo[1].bDead, pJunqi->aInfo[3].bDead);
		}
	}
}
...
{
	int i;
	enum ChessDir old_turn = pJunqi->eTurn;
	// Skip dead players, check up to 3 times to handle 3 players dead
	for(i=0; i<3; i++)
	{
	...
	LOG_TRACE(LOG_CAT_CORE, "ChessTurn: %d -> %d (skipped %d dead)",
	          old_turn, pJunqi->eTurn, i);
}
...
void IncJumpCnt(Junqi *pJunqi, int iDir)
{
	...
	LOG_DEBUG(LOG_CAT_CORE, "IncJumpCnt: player %d jump_cnt=%d", iDir, cntJump);

	if( cntJump==5 )
	{
		LOG_INFO(LOG_CAT_CORE, "player %d jumped 5 times -> dead", iDir);
...
// Check if a player has no valid moves, if so mark as dead
int CheckIfDead(Junqi *pJunqi, int iDir)
{
	...
	for(i=0; i<30; i++)
	{
		...
		if( pLineup->type==JUNQI || pLineup->type==DILEI )
		{
			continue;
		}
		for(j=0; j<129; j++)
		{
			...
			if( pTemp->type!=NONE && pLineup->iDir%2==pTemp->pLineup->iDir%2 )
				continue;
			if( IsEnableMove(pJunqi, pLineup->pChess, pTemp) )
				return 0;
		}
	}

	DestroyAllChess(pJunqi, iDir);
	LOG_DEBUG(LOG_CAT_RULE, "CheckIfDead: player %d has no legal move, mark dead", iDir);
	return 1;
}
