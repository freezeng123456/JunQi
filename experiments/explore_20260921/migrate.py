...
def migrate(source,output,kind):
    ...
    if kind=='F':cfg['tactical_features']=True
    elif kind=='D':cfg['depth']+=2
    ...
    if kind=='D':
        for block in list(new.transformer)[old.cfg.depth:]:
            torch.nn.init.zeros_(block.attn.out_proj.weight);torch.nn.init.zeros_(block.attn.out_proj.bias)
            torch.nn.init.zeros_(block.ff[3].weight);torch.nn.init.zeros_(block.ff[3].bias)
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('source');ap.add_argument('output');ap.add_argument('kind',choices=['F','D']);a=ap.parse_args()
