...
def make_report(root, result):
    ...
    sections += ['## 实验覆盖与共同基线', table(
        ...
        '本轮核心网络实际读取 317 通道空间观测和观察者席位，没有读取单独的 28 维 global 向量；新增特征也没有补入独立的全局身份计数。因此，这项比较覆盖当前 BeliefNet 输入接口，不能代表所有公开信息表示或网络结构的能力上限。',
    ...
    sections += ['## 长期轨迹在哪些局面得到检验',
        ...
        '距离统计使用公开移动端点的曼哈顿位移，没有还原铁路拐弯的完整行走路径。通道 2 与 12 是重复的“曾移动”标志，因此 32 通道不等于 32 个独立统计量。方向分量保持世界坐标，整张特征图按观察者旋转。完整定义及这些冻结版本边界见同目录 FEATURE_CATALOG.md。',
