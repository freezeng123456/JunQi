...
void SetChessImageType(Junqi *pJunqi, int dir, int i, int iType)
{
	...
	pRotate270 = gdk_pixbuf_rotate_simple(
			pRotate90, GDK_PIXBUF_ROTATE_UPSIDEDOWN);
	/* Every direction owns a separate widget.  The old code aliased [0]/[2]
	 * and later destroyed/unref'd the same GtkImage twice. */
	pJunqi->Lineup[dir][i].pImage[0] = gtk_image_new_from_pixbuf(pPixbuf);
	pJunqi->Lineup[dir][i].pImage[1] = gtk_image_new_from_pixbuf(pRotate90);
	pJunqi->Lineup[dir][i].pImage[2] = gtk_image_new_from_pixbuf(pPixbuf);
	pJunqi->Lineup[dir][i].pImage[3] = gtk_image_new_from_pixbuf(pRotate270);
	for (int d = 0; d < 4; d++)
	{
		g_object_ref_sink(pJunqi->Lineup[dir][i].pImage[d]);
	}
	g_object_unref(pRotate90);
	g_object_unref(pRotate270);
}
...
void SetChess(Junqi *pJunqi, enum ChessDir dir)
{
	...
	for(i=0;i<30;i++)
	{
		...
		if(iType!=NONE)
		{
			...
			{
				if (pJunqi->eShowMode == SHOW_MODE_DARK)
				{
					if (dir != HOME) isDark = 1;
				}
				else if (pJunqi->eShowMode == SHOW_MODE_HALF_DARK)
				{
					if (dir == RIGHT || dir == LEFT) isDark = 1;
				}
			}
			if (isDark)
			{
			...
			if( pJunqi->Lineup[dir][i].pFlag )
			{
				...
				pJunqi->Lineup[dir][i].pFlag = NULL;
			}
...
BoardChess *GetChessPos(Junqi *pJunqi, int x, int y)
{
	...
	if( (x>=MIDX) && (x<= MIDX+HORIZONTAL_LONG1)  )
	{
		if( (y>=BOTTOMY) && (y<=BOTTOMY+VERTICAL_LONG) )
		{
			...
			if( (x<=MIDX+iPosX*LENGTH2+36) &&
					(y<=BOTTOMY+iPosY*LENGTH1+27) )
			{
				iPos = iPosY*5 + iPosX;
		...
		else if( (y>=NINE_GRID_Y) && (y<=NINE_GRID_Y+HORIZONTAL_LONG2) )
		{
			...
			if( (x<=MIDX+iPosX*2*LENGTH2+36) &&
					(y<=NINE_GRID_Y+iPosY*2*LENGTH1+27) )
			{
				iPos = iPosY*3 + iPosX;
	...
	else if( (y>=MIDY) && (y<=MIDY+HORIZONTAL_LONG2)  )
	{
		if((x>=RIGHTX) && (x<= RIGHTX+VERTICAL_LONG))
		{
			...
			assert(iPosY>=0 && iPosY<=4);
			if( (x<=RIGHTX+iPosX*LENGTH1+27) &&
					(y<=MIDY+iPosY*LENGTH1+36) )
			{
...
int AssertChess(BoardChess *pChess)
{
	int rc = 1;
...
int AssertLineup(ChessLineup *pLineup)
{
	int rc = 1;
...
int assertAllLineup(Junqi *pJunqi)
{
	...
	for(i=0;i<4;i++)
	{
		for(j=0;j<30;j++)
		{
			rc = AssertLineup(&pJunqi->Lineup[i][j]);
			if(!rc) return 0;
...
void DestroyAllChess(Junqi *pJunqi, int iDir)
{
	...
	for(i=0; i<30; i++)
	{
		...
		if( pLineup->type!=NONE )
		{
			...
			if (pLineup->pFlag)
			{
				gtk_widget_destroy(pLineup->pFlag);
				pLineup->pFlag = NULL;
			}
			for(j=0; j<4; j++)
			{
	...
	// Check if the game is over: both players of one team are dead
	if( !pJunqi->bReplay && !pJunqi->bAnalyse )
	{
		// Team 0,2 vs Team 1,3
		if( (pJunqi->aInfo[0].bDead && pJunqi->aInfo[2].bDead) ||
		    (pJunqi->aInfo[1].bDead && pJunqi->aInfo[3].bDead) )
		{
			pJunqi->bStart = 0;
		}
	}
...
{
	if( aBuf == NULL || size <= 0 || pJunqi->iReOfst < 0 ||
			pJunqi->iReOfst > PAGE_SIZE - size )
	{
		log_b("replay buffer full: offset=%d size=%d", pJunqi->iReOfst, size);
		return;
	}
...
{
	if (aBuf == NULL || iOfst < 0 || iAmt < 0 || iOfst > PAGE_SIZE - iAmt)
		return;
...
//	assert( AssertChess(pSrc) );
//	assert( AssertChess(pDst) );
//	assert( AssertLineup(pSrc->pLineup) );
//	assert( AssertLineup(pDst->pLineup) );
//	assert( assertAllLineup(pJunqi) );
}
...
void LoadLineup(Junqi *pJunqi, int iDir, u8 *chess)
{
	...
	for(i=0;i<30;i++)
	{
		...
		assert( AssertChess(&pJunqi->ChessPos[iDir][i]) );
	}
                  ...
                  gpointer         user_data)
{
	...
	if (response_id == GTK_RESPONSE_ACCEPT)
	{
		...
		if (fd < 0)
		{
			ShowDialogMessage(pJunqi, "无法打开布阵文件", 0);
			goto lineup_done;
		}
        ...
        gpointer         user_data)
{
	...
	if (response_id == GTK_RESPONSE_ACCEPT)
	{
		...
		if (fd < 0)
		{
			ShowDialogMessage(pJunqi, "无法打开复盘文件", 0);
			goto replay_done;
		}
		...
		if( bytes_read >= MOVE_OFFSET && memcmp(aBuf, aMagic, 4)==0 )
		{
			...
			pJunqi->bStop = 1;
			pJunqi->iRpStep = 0;
			/* Force ShowReplayStep to reset its cached cursor when a new file is
			 * opened (the legacy cache was function-static). */
			pJunqi->bResetFlag = 1;
		}
		else
		{
			ShowDialogMessage(pJunqi, "不是有效的复盘文件", 0);
		}
	}
...
void ShowReplayStep(Junqi *pJunqi, u8 next_flag)
{
	...
	for(i=preStep; i<pJunqi->iRpStep; i++)
	{
		...
		if( *((u8*)(pJunqi->aReplay+MOVE_OFFSET+4*i))==PLAY_EVENT )
		{
			...
			/* Keep the event type intact.  The old code sent SURRENDER_EVENT
			 * even for a recorded jump, which desynchronised analysis engines. */
			pJunqi->addr = pJunqi->addr_tmp[0];
        ...
        gpointer         user_data)
{
	...
	if (response_id == GTK_RESPONSE_ACCEPT)
	{
		...
		if (fd < 0)
		{
			ShowDialogMessage(pJunqi, "无法保存布阵文件", 0);
			goto lineup_save_done;
		}
        ...
        gpointer         user_data)
{
	...
	if (response_id == GTK_RESPONSE_ACCEPT)
	{
		...
		if (fd < 0)
		{
			ShowDialogMessage(pJunqi, "无法保存复盘文件", 0);
...
void RefreshChessBoardDisplay(Junqi *pJunqi)
{
    int i, j;
    enum ChessType iType;
    ...
    for(i=0; i<4; i++) // dir
    {
        ...
        {
            ChessLineup *pLineup = &pJunqi->Lineup[i][j];
            if(pLineup->type == NONE) continue;
            ...
            if(pLineup->pImage[0])
                gtk_image_set_from_pixbuf(GTK_IMAGE(pLineup->pImage[0]), basePixbuf);
            if(pLineup->pImage[2])
                gtk_image_set_from_pixbuf(GTK_IMAGE(pLineup->pImage[2]), basePixbuf);
            
            // 1 (RIGHT) - 90度逆时针
            rot90 = gdk_pixbuf_rotate_simple(basePixbuf, GDK_PIXBUF_ROTATE_COUNTERCLOCKWISE);
            if(pLineup->pImage[1])
                gtk_image_set_from_pixbuf(GTK_IMAGE(pLineup->pImage[1]), rot90);
            
            // 3 (LEFT) - 270度逆时针 (90 + 180)
            rot270 = gdk_pixbuf_rotate_simple(rot90, GDK_PIXBUF_ROTATE_UPSIDEDOWN);
            if(pLineup->pImage[3])
                gtk_image_set_from_pixbuf(GTK_IMAGE(pLineup->pImage[3]), rot270);
            
            g_object_unref(rot90);
            g_object_unref(rot270);
        }
    }
}
