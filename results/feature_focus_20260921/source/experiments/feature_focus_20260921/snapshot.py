...
def main():
 ...
 extra={k:bool(not torch.equal(v,base['policy'][k])) for k,v in state['policy'].items() if k.startswith(('tactical_head.','relational_head.'))}
 report={'added_module_tensor_changes':extra,'status':'finite_strict_reload_verified','sha256':before,'rollouts':state['num_rollout'],'optimizer_updates':state['num_train_step'],'finite_tensors':count[0],'policy_tensors':len(state['policy']),'parameters':sum(p.numel() for p in model.parameters()),'changed_inherited_tensors':changed,'config':cfg}
